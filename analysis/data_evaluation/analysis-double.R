...
compare = read.csv("./input/WOFF2REN.csv", encoding = "UTF-8")
compare$format <- "compare"

data <- rbind(woff2, compare)
data <- data %>%
...
violin_length <- ggplot(data, aes(x = format, y = data$LENGTH, fill = format, color = format)) +
  ...
  scale_fill_manual(values = c(woff2 = "#DCC7AA", compare = "#6b7a8f"), guide = "none") +
  scale_color_manual(values = c(woff2 = "#DCC7AA", compare = "#6b7a8f"), guide = "none") +
  theme(legend.position = c(0.88, 0.8)) +
  geom_boxplot(width = 0.2, fill = c("#6b7a8f", "#DCC7AA"), color = "black", outlier.shape = NA)
...
density_length <- ggplot(data, aes(x = LENGTH)) +
  ...
  scale_fill_manual(values = c(woff2 = "#DCC7AA", compare = "#6b7a8f")) +
  scale_color_manual(values = c(woff2 = "#DCC7AA", compare = "#6b7a8f")) +
...
violin_cpu <- ggplot(data, aes(x = format, y = CPUCYCLES, fill = format, color = format)) +
  ...
  geom_boxplot(width = 0.2, fill = c("#DCC7AA", "#6b7a8f"), color = "black", outlier.shape = NA)
