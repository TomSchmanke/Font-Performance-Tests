...
compare = read.csv("./input/WOFF2ALL.csv", encoding = "UTF-8")
compare$format <- "compare"

data <- rbind(otf, compare)
data <- data %>%
...
violin_length <- ggplot(data, aes(x = format, y = data$LENGTH, fill = format, color = format)) +
  ...
  scale_fill_manual(values = c(otf = "#DCC7AA", compare = "#6b7a8f"), guide = "none") +
  scale_color_manual(values = c(otf = "#DCC7AA", compare = "#6b7a8f"), guide = "none") +
...
density_length <- ggplot(data, aes(x = LENGTH)) +
  ...
  scale_fill_manual(values = c(otf = "#DCC7AA", compare = "#6b7a8f")) +
  scale_color_manual(values = c(otf = "#DCC7AA", compare = "#6b7a8f")) +
